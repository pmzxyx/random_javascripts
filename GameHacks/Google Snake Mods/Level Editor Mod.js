//bookmarklet:
javascript:function processCommonCode(code){if(window.commonHasAlreadyRun){console.log("Notice: processCommonCode has already been ran once. This has be prevented from running a second time.");return}window.commonHasAlreadyRun=!0,window.tileWidth=code.match(/[a-z]\.[$a-zA-Z0-9_]{0,6}\.fillRect\([a-z]\*[a-z]\.[$a-zA-Z0-9_]{0,6}\.([$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}),[a-z]\*[a-z]\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6},[a-z]\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6},[a-z]\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}\)/)[1],[,window.applePosProperty,window.appleSpeedProperty]=code.match(/&&\([$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}\.x&&\([$a-zA-Z0-9_]{0,6}\.([$a-zA-Z0-9_]{0,6})\.x\+=[$a-zA-Z0-9_]{0,6}\.([$a-zA-Z0-9_]{0,6})\.x\),/),window.bodyArray=code.match(/var [a-z]=this\.[$a-zA-Z0-9_]{0,6}\.([$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6})\[0\]\.clone\(\);/)[1],window.makeApple=code.match(/this\.[$a-zA-Z0-9_]{0,6}\.push\(([$a-zA-Z0-9_]{0,6})\(this,-5,-4\)\)/)[1],window.appleArray=code.match(/this\.([$a-zA-Z0-9_]{0,6})\.push\([$a-zA-Z0-9_]{0,6}\(this,-6,-3\)\)/)[1],window.appleArrayHolderOfWholeSnakeObject=code.match(/this\.([$a-zA-Z0-9_]{0,6})\.reset\(\);this\.[$a-zA-Z0-9_]{0,6}=!1;/)[1],window.coordConstructor=code.match(/new ([$a-zA-Z0-9_]{0,6})\(1,1\)/)[1],window.boardDimensions=code.match(/x===Math.floor\([a-z]\.[$a-zA-Z0-9_]{0,6}\.([$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6})\.width\/2\)&&/)[1],window.mainClass=code.match(/([$a-zA-Z0-9_]{0,6})=function\(a,b,c\){this\.settings=[a-z];this\.menu=[a-z];this\.header=[a-z];/)[1];let funcWithReset=findFunctionInCode(code,/[$a-zA-Z0-9_]{0,6}\.reset=function\(\)$/,/a=\n?\.66/,!1);funcWithReset=assertReplace(funcWithReset,"{","{wholeSnakeObject = this;"),eval(funcWithReset=swapInMainClassPrototype(mainClass,funcWithReset=assertReplace(funcWithReset,/[$a-zA-Z0-9_]{0,6}\([a-z]\.settings\)&&\([a-z]\.[$a-zA-Z0-9_]{0,6}\.[$a-zA-Z0-9_]{0,6}=!0\)\)/,"$&;simpleHookManager.runHook('afterResetBoard')")));let funcWithResetState=findFunctionInCode(code,/[$a-zA-Z0-9_]{0,6}\.prototype\.resetState=function\(a\)$/,/void 0===[a-z]\?!0:[a-z];this\.[$a-zA-Z0-9_]{0,6}\.reset\(\);/);eval(funcWithResetState=assertReplace(funcWithResetState,"{","{megaWholeSnakeObject = this;"))}window.commonHasAlreadyRun=window.commonHasAlreadyRun||!1,window.snake.runMod=function(e){let t=document.body.getElementsByTagName("script");for(let o of t){if(""==o.src||-1!=o.src.indexOf("apis.google.com"))continue;let a=new XMLHttpRequest;a.open("GET",o.src),a.onload=function(){-1!==this.responseText.indexOf("trophy")&&e(this.responseText)},a.send()}};var wholeSnakeObject,megaWholeSnakeObject,simpleHookManager={hooks:{afterResetBoard:[]},runHook:function(e){if(this.hooks.hasOwnProperty(e)&&Array.isArray(this.hooks[e]))for(let t of this.hooks[e])t.callback();else throw Error(`Hook type "${e}" not found`)},registerHook:function(e,t,o){if(!this.hooks.hasOwnProperty(e)||!Array.isArray(this.hooks[e]))throw Error("Hook type not found");if("string"!=typeof t||"function"!=typeof o)throw Error("hookName must be a string, hookCallback must be a function.");this.hooks[e].some(e=>e.name===t)||this.hooks[e].push({name:t,callback:o})}};function swapInMainClassPrototype(e,t){if(/^[$a-zA-Z0-9_]{0,6}=function/.test(t))throw Error("Error, function is of form abc=function(), but this only works for stuff like s_.abc=function()");return t=assertReplace(t,/^[$a-zA-Z0-9_]{0,6}/,`${e}.prototype`)}function findFunctionInCode(e,t,o,a=!1){let n=t.source,r=t.flags;if("$"!==n[n.length-1])throw Error("functionSignature regex should end in $");n.replaceAll(/,|=/g,"$&\\n?"),t=RegExp(n,r);let s=e.search(o);-1==s&&(console.log("%cCouldn't find a match for somethingInsideFunction","color:red;"),diagnoseRegexError(e,o)),startIndex=0;for(let i=s;i>=0&&-1===(startIndex=e.substring(0,i).search(t));i--)if(0==i)throw Error("Couldn't find function signature");let l=0,c=!1,h=0,$=e.length;for(let f=startIndex;f<=$;f++){if(c||"{"!=e[f]||(c=!0),"{"==e[f]&&l++,"}"==e[f]&&l--,c&&0==l){h=f;break}if(f==$)throw Error("Couldn't pair up brackets")}let u=e.substring(startIndex,h+1);if(-1===u.search(o))throw Error("Function signature does not belong to the same function as somethingInsideFunction");return a&&console.log(u),u}function assertReplace(e,t,o){if("string"!=typeof e)throw Error("String argument expected for assertReplace");let a=e.replace(t,o);return e===a&&diagnoseRegexError(e,t),a}function assertReplaceAll(e,t,o){if("string"!=typeof e)throw Error("String argument expected for assertReplace");let a=e.replaceAll(t,o);return e===a&&diagnoseRegexError(e,t),a}function diagnoseRegexError(e,t){if(!(t instanceof RegExp))throw Error("Failed to find match using string argument. No more details available");let o=e.replaceAll(/\n/g,"");if(!t.test(o))throw Error("Failed to find match for regex.");let a=t.source,n=t.flags;for(let r of["%","&","\\*","\\+",",","-","\\/",":",";","<","=",">","\\?","{","\\|","}"])for(let s=a.indexOf(r);-1!==s;s=a.indexOf(r,s+1)){let i=`${a.slice(0,s+r.length)}\\n?${a.slice(s+r.length)}`,l;try{l=RegExp(i,n)}catch(c){continue}if(l.test(e))throw console.log(`Suggested regex improvement:${l}`),Error("Suggested improvement found! Error with line break, failed to find match for regex. See logged output for regex to use instead that should hopefully fix this.")}throw Error("Line break error! Failed to failed to find match for regex - most likely caused by a new line break. No suggestions provided")}snake.runMod(processCommonCode);
